...
function Sidebar() {
    ...
    const { logout, onlineUser } = React.useContext(AuthContext);
        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
            <div className='pb-5'>
                <div className='flex justify-between items-center'>
                    ...
                    <div className='relative py-2 group'>
                        ...
                        <div className='absolute top-full right-0 z-20- w-32 p-5 rounded-md bg-black border border-white-600
                ...
                <div className='bg-[#000000] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
            ...
            <div className='flex flex-col'>
                { filteredusers.map((user, index) => (
                    ...
                    ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`} onClick={() => {setSelectedUser(user); setUnseenMessages(prev => ({...prev, [user._id] : 0}))}}>
                        ...
                        {unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-white text-black'>{unseenMessages[user._id]}</p>}
                    </div>
                )) }
            </div>
        </div>
